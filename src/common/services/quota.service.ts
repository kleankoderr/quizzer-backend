...
export class QuotaService {
  ...
  async getQuotaStatus(userId: string) {
    ...
    return {
      ...
      summary: {
        used: userQuota.monthlySummaryCount,
        limit: planQuotas.summaries || 0,
        remaining: Math.max(
          0,
          (planQuotas.summaries || 0) - userQuota.monthlySummaryCount
        ),
      },
      fileUpload: {
  ...
  private getCurrentUsage(userQuota: any, feature: QuotaFeature): number {
    const usageMap: Record<QuotaFeature, number> = {
      ...
      summary: userQuota.monthlySummaryCount,
    };
