...
const MODEL_MAPPING: Record<string, string> = {
  quiz: MODELS.LITE, // Lite model for quizzes
  flashcard: MODELS.LITE, // Lite model for flashcards
  'learning-guide': MODELS.PREMIUM, // Premium model for learning guides
  explanation: MODELS.LITE, // Lite model for explanations
  example: MODELS.LITE, // Lite model for examples
  recommendation: MODELS.STANDARD, // Standard model for recommendations
  companion: MODELS.LITE, // Lite model for companion interactions
  content: MODELS.LITE, // Lite model for generic content
};
...
export class GeminiAiService extends AiService {
  ...
  constructor(
    ...
    if (apiKey) {
      ...
      this.logger.log('Initialized AI service with Google AI Studio (API Key)');
    } else {
      throw new Error('GOOGLE_API_KEY configuration found');
    }
